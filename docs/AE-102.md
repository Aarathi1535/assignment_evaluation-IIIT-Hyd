...
### 1.2.1 Change Stream Fallback

MongoDB Change Streams are the primary event backbone when the connected
MongoDB deployment supports them.

If Change Streams are unavailable, the service falls back to an in-process
`EventEmitter`. This fallback exists to support environments such as
standalone MongoDB instances and test databases that do not support Change
Streams.

The fallback has a narrower delivery guarantee:
...
* Clients connected to other application instances do not receive the
  in-process event.
* This fallback therefore does **not** provide cross-instance live progress
  delivery.
* The service logs a warning when Change Stream initialization fails.
* Clients can use the existing progress REST endpoint as a polling fallback
