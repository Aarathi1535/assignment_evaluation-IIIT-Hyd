 ...
 * GET /api/scripts/[id]
 *
 * Retrieves answer script metadata and its ordered pages for grading evaluation.
...
export async function GET(
  ...
  try {
    ...
    // 5. Serialize / anonymize answer script metadata for viewer
    const serializedScripts = await Anonymizer.serializeAnswerScripts(
      [script],
      { id: user.id, role: user.role }
    );
    const serializedScript = serializedScripts[0] || {
