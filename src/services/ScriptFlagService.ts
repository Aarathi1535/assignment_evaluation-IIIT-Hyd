...
export interface EffectiveGradeResult {
    ...
    override?: IScriptFlagResolution | null;
...
export class ScriptFlagService {
    ...
    static async getEffectiveGrade(
        ...
        const originalGrade = await Grade.findOne(gradeQuery);
        ...
        const overrideFlag = await ScriptFlag.findOne(flagQuery).sort({ 'resolution.at': -1, updatedAt: -1 });
        ...
        if (overrideFlag?.resolution && typeof overrideFlag.resolution.newScore === 'number') {
            return {
                ...
                override: overrideFlag.resolution,
